...
The following diagram lays out the molecule lifecycle in more detail.
...
* Your molecule values may be re-mounted. `onMounted` may be called again after `onUnmounted` has been called.
* Lifecycles methods will be called in order, so `onMounted` then `onUmounted` then again `onMounted`, and so on.
* Your molecule values may NEVER be mounted. Strict mode may dispose of a value before calling `onMounted`.
* In strict mode your molecule may be called twice:
  * one value thrown away (in the first render)
  * the other value `onMounted` then `onUmounted` then again `onMounted` (in the second render)
